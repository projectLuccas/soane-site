...
function voltarAoMenu() {
    ...
    document.getElementById("paginaSurpresinhaSoso").style.display = "none";
    document.getElementById("jumpscareSoso").classList.remove("ativo");
    document.getElementById("posJumpscareSoso").style.display = "none";
...
let timerJumpscareSoso = null;
let timerFinalJumpscareSoso = null;


function abrirSurpresinhaSoso() {

    document.getElementById("menu").style.display = "none";

    document.getElementById("paginaSurpresinhaSoso").style.display = "flex";

    document.getElementById("jumpscareSoso").classList.remove("ativo");

    document.getElementById("posJumpscareSoso").style.display = "none";

    document.getElementById("textoSurpresinha").innerHTML =
        "Eu preparei uma coisinha especialmente pra você, neguinha 🥺❤️";

    const botao =
        document.getElementById("botaoAbrirSurpresa");

    botao.style.display = "block";
    botao.disabled = false;
    botao.textContent = "💖 Ver minha surpresa";

    window.scrollTo(0, 0);

    salvarTelaAtual("surpresinhaSoso");
}


function iniciarSurpresinhaSoso() {

    const botao =
        document.getElementById("botaoAbrirSurpresa");

    const texto =
        document.getElementById("textoSurpresinha");

    const audio =
        document.getElementById("audioJumpscareSoso");
    ...
    texto.innerHTML =
        "Só um pouquinho... 🥺<br>Eu fiz com carinho ❤️";


    /*
       O play começa a partir do clique dela.
       Deixamos mudo durante a espera para
       melhorar a compatibilidade no celular.
    */
    ...
    const promessaAudio = audio.play();

    if (promessaAudio !== undefined) {

        promessaAudio.catch(() => {
            // Se o navegador bloquear, o susto visual continua funcionando.
        });

    }


    clearTimeout(timerJumpscareSoso);


    timerJumpscareSoso = setTimeout(() => {

        dispararJumpscareSoso();
...
function dispararJumpscareSoso() {

    const jumpscare =
        document.getElementById("jumpscareSoso");

    const audio =
        document.getElementById("audioJumpscareSoso");


    jumpscare.classList.add("ativo");


    /*
       O aparelho continua controlando
       o volume físico final.
    */
    ...
    /*
       Vibração curta quando o navegador
       e o celular permitirem.
    */
    ...
    clearTimeout(timerFinalJumpscareSoso);


    timerFinalJumpscareSoso = setTimeout(() => {

        jumpscare.classList.remove("ativo");
